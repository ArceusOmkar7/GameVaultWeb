...
public class OrderManagement {
    ...
    public void placeOrder(int userId) throws CartEmptyException, UserNotFoundException, IllegalStateException {
        ...
        if (user.getWalletBalance() < totalAmount) {
            throw new IllegalStateException("Insufficient balance. Your balance is " + user.getWalletBalance()
                    + ", but the total is " + totalAmount);
        }
        ...
        // NOTE: True atomicity requires database transactions, which DBUtil doesn't
        // manage.
        ...
        try {
            ...
            int rowsAffectedUser = DBUtil.executeUpdate(sqlUpdateUser, totalAmount, userId, totalAmount); // Add balance
                                                                                                          // check
            if (rowsAffectedUser == 0) {
                ...
                if (checkUser != null && checkUser.getWalletBalance() < totalAmount) {
                    throw new IllegalStateException("Insufficient balance. Your balance is "
                            + checkUser.getWalletBalance() + ", but the total is " + totalAmount);
                }
                // Some other reason update failed
                throw new IllegalStateException(
                        "Failed to update user balance. Possibly concurrent update or user not found.");
            }
            ...
            Transaction transaction = new Transaction(null, savedOrder.getOrderId(), userId, "Purchase",
                    (float) totalAmount,
                    LocalDateTime.now());
            ...
            if (savedOrder == null) { // Failed before or during order save
                try {
                    ...
                    System.err.println("FATAL: Failed to rollback user balance! Manual correction needed for userId: "
                            + userId + ", amount: " + totalAmount);
                    // Log this critical failure
                }
            }
            // If order was saved but items/transaction failed, deleting the order might be
            // an option,
            ...
            throw new IllegalStateException("Order processing failed due to a database error. Please try again later.",
                    e);
