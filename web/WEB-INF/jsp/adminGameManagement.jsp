...
<body class="bg-gray-100">
                ...
                <div class="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
                    <div>
                        <p class="text-sm text-gray-700">
                            Showing <span class="font-medium">${firstGame}</span> to <span class="font-medium">${lastGame}</span> of <span class="font-medium">${totalGames}</span> results
                        </p>
                    </div>
                    <div>
                            ...
                            <nav class="relative z-0 inline-flex rounded-md shadow-sm -space-x-px" aria-label="Pagination">
                                ...
                                <c:choose>
                                    <c:when test="${currentPage > 1}">
                                        <a href="${pageContext.request.contextPath}${baseUrl}page=${currentPage - 1}" class="relative inline-flex items-center px-2 py-2 rounded-l-md border border-gray-300 bg-white text-sm font-medium text-gray-500 hover:bg-gray-50">
                                ...
                                <c:if test="${startPage > 1}">
                                    <a href="${pageContext.request.contextPath}${baseUrl}page=1" class="relative inline-flex items-center px-4 py-2 border border-gray-300 bg-white text-sm font-medium text-gray-500 hover:bg-gray-50">1</a>
                                ...
                                <c:forEach begin="${startPage}" end="${endPage}" var="i">
                                    <c:choose>
                                        ...
                                        <c:otherwise>
                                            <a href="${pageContext.request.contextPath}${baseUrl}page=${i}" class="relative inline-flex items-center px-4 py-2 border border-gray-300 bg-white text-sm font-medium text-gray-500 hover:bg-gray-50">${i}</a>
                                        </c:otherwise>
                                    </c:choose>
                                </c:forEach>
                                ...
                                <c:if test="${endPage < totalPages}">
                                    ...
                                    <a href="${pageContext.request.contextPath}${baseUrl}page=${totalPages}" class="relative inline-flex items-center px-4 py-2 border border-gray-300 bg-white text-sm font-medium text-gray-500 hover:bg-gray-50">${totalPages}</a>
                                </c:if>
                                
                                <!-- Next page link -->
                                <c:choose>
                                    <c:when test="${currentPage < totalPages}">
                                        <a href="${pageContext.request.contextPath}${baseUrl}page=${currentPage + 1}" class="relative inline-flex items-center px-2 py-2 rounded-r-md border border-gray-300 bg-white text-sm font-medium text-gray-500 hover:bg-gray-50">
    ...
    <!-- Edit Game Modal -->
    <div id="editGameModal" class="fixed inset-0 bg-gray-800 bg-opacity-75 flex items-center justify-center z-50 hidden">
        <div class="bg-white rounded-lg shadow-lg w-full max-w-3xl max-h-screen overflow-y-auto">
            <div class="px-6 py-4 border-b border-gray-200 flex justify-between items-center">
                ...
                <form id="editGameForm" action="${pageContext.request.contextPath}/admin/edit-game" method="post">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div class="col-span-2">
                            ...
                            <input type="text" name="title" id="editTitle" class="mt-1 focus:ring-blue-500 focus:border-blue-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md" required>
                        </div>
                        ...
                        <div>
                            <label for="editDeveloper" class="block text-sm font-medium text-gray-700">Developer</label>
                            <input type="text" name="developer" id="editDeveloper" class="mt-1 focus:ring-blue-500 focus:border-blue-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
                        </div>
                        ...
                        <div>
                            ...
                            <div class="mt-1 relative rounded-md shadow-sm">
                                ...
                                <input type="number" step="0.01" min="0" name="price" id="editPrice" class="focus:ring-blue-500 focus:border-blue-500 block w-full pl-7 pr-12 sm:text-sm border-gray-300 rounded-md" placeholder="0.00" required>
                            </div>
                        </div>
                        
                        <div>
                            <label for="editPlatform" class="block text-sm font-medium text-gray-700">Platform</label>
                            <input type="text" name="platform" id="editPlatform" class="mt-1 focus:ring-blue-500 focus:border-blue-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md" placeholder="PC, PlayStation 5, Xbox Series X/S">
                        </div>
                        
                        <div>
                            <label for="editGenre" class="block text-sm font-medium text-gray-700">Genre</label>
                            <input type="text" name="genre" id="editGenre" class="mt-1 focus:ring-blue-500 focus:border-blue-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md" placeholder="Action, Adventure, RPG">
                        </div>
                        
                        <div>
                            <label for="editReleaseDate" class="block text-sm font-medium text-gray-700">Release Date</label>
                            <input type="date" name="releaseDate" id="editReleaseDate" class="mt-1 focus:ring-blue-500 focus:border-blue-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
                        </div>
                        
                        <div>
                            <label for="editRating" class="block text-sm font-medium text-gray-700">Rating</label>
                            <input type="number" step="0.1" min="0" max="5" name="rating" id="editRating" class="mt-1 focus:ring-blue-500 focus:border-blue-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md" placeholder="0.0-5.0">
                        </div>
                        
                        <div class="col-span-2">
                            <label for="editImagePath" class="block text-sm font-medium text-gray-700">Image Path</label>
                            <input type="text" name="imagePath" id="editImagePath" class="mt-1 focus:ring-blue-500 focus:border-blue-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md" placeholder="URL or path to image">
                            <p class="mt-1 text-xs text-gray-500">Enter a URL or upload an image separately via the image upload tool</p>
                        </div>
                        
                        <div class="col-span-2">
                            <div class="flex items-center space-x-2 mt-4">
                                <div id="currentImageContainer" class="bg-gray-100 p-2 rounded hidden">
                                    <p class="text-sm font-medium text-gray-700 mb-1">Current Image:</p>
                                    <img id="currentGameImage" src="" alt="" class="h-20 w-auto object-cover rounded">
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="mt-6 flex justify-end">
                        <button type="button" id="cancelEditGame" class="bg-white py-2 px-4 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 mr-3">
                            Cancel
                        </button>
                        <button type="submit" class="inline-flex justify-center py-2 px-4 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500">
                            Save Changes
    ...
    <script>
        ...
        applyFiltersBtn.addEventListener('click', () => {
            ...
            let url = '${pageContext.request.contextPath}/admin/game-management?';
            ...
            if (platformFilter) {
                url += `platformFilter=${encodeURIComponent(platformFilter)}&`;
            }
            
            if (genreFilter) {
                url += `genreFilter=${encodeURIComponent(genreFilter)}&`;
            }
        ...
        const editButtons = document.querySelectorAll('.text-blue-600.hover\\:text-blue-900.mr-3');
        ...
        async function loadGameForEdit(gameId) {
            try {
                const response = await fetch(`${pageContext.request.contextPath}/admin/edit-game?id=${gameId}&ajax=true`);
                if (!response.ok) {
                ...
                document.getElementById('editReleaseDate').value = formatDateForInput(game.releaseDate);
