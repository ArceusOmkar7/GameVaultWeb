...
<div class="gv-card">
  <div class="gv-card-header">
    <h1 class="text-xl font-bold">JSON Game Data Loader</h1>
  </div>
  <div class="gv-card-body">
    <c:choose>
      <c:when test="${dataLoaded && !forceReload}">
        ...
        <p>If you want to reload the data, click the button below:</p>
        <a
...
${stackTrace}</pre
        ...
        <c:if test="${generationSuccess}">
          <div class="gv-alert gv-alert-success mb-4">
            <p>
              Successfully loaded ${successCount} of ${totalCount} games from
              JSON file!
            </p>
          </div>
        </c:if>
